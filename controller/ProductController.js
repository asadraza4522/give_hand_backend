...
const uniqueName = (name, id) => new Promise((resolve, reject) => {
...
const viewProducts = (req, res) => {
    ...
    if (search !== '') {
        fields.push({ $or: [{ name: { $regex: search, $options: 'i' } }, { tags: { $regex: search, $options: 'i' } }] })
    }
    ...
    try {
        Product.paginate(fields.length > 0 ? { $and: fields } : {}, { page: req.query.page, limit: 10 })
            .then(result => {
                response.data = result
...
const liveSearch = (req, res) => {
    ...
    try {
        ...
        Product.find({ $or: [{ "name": { $regex: req?.params?.search, $options: 'i' } }, { "tags": { $regex: req?.params?.search, $options: 'i' } }] })
            .limit(20)
            .then((result) => {
    ...
    try {
        ...
        uniqueName(req.body.name, req.body._id)
            .then((result) => {
                ...
                Product.findByIdAndUpdate({ "_id": req.body._id }, req.body, (error, data) => {
                    if (error) {
